...
public class RealmsDownloadLatestWorldScreen extends RealmsScreen {
	...
	private void checkDownloadSize() {
		if (!this.finished && !this.checked) {
			...
			if (this.getContentLength(this.worldDownload.downloadLink()) >= 5368709120L) {
				Component popupMessage = Component.translatable("mco.download.confirmation.oversized", new Object[]{Unit.humanReadable(5368709120L)});
	...
	private Component createProgressNarrationMessage() {
		...
		if (this.progress != null) {
			elements.add(Component.translatable("mco.download.percent", new Object[]{this.progress}));
			elements.add(Component.translatable("mco.download.speed.narration", new Object[]{Unit.humanReadable(this.bytesPersSecond)}));
		}
	...
	private void extractProgressBar(final GuiGraphicsExtractor graphics) {
		...
		graphics.centeredText(this.font, Component.translatable("mco.download.percent", new Object[]{this.progress}), this.width / 2, 84, -1);
	}
	...
	private void extractDownloadSpeed0(final GuiGraphicsExtractor graphics, final long bytesPerSecond) {
		if (bytesPerSecond > 0L) {
			...
			graphics.text(
				this.font, Component.translatable("mco.download.speed", new Object[]{Unit.humanReadable(bytesPerSecond)}), this.width / 2 + progressLength / 2 + 15, 84, -1
			);
		}
	}
