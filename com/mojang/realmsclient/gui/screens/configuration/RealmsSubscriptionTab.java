...
class RealmsSubscriptionTab extends GridLayoutTab implements RealmsConfigurationTab {
		...
		this.deleteButton = helper.addChild(
			Button.builder(
					...
					button -> minecraft.setScreen(
						RealmsPopups.warningPopupScreen(configurationScreen, Component.translatable("mco.configure.world.delete.question.line1"), popup -> this.deleteRealm())
					)
				)
				.bounds(0, 0, 200, 20)
				.build()
	...
	private Component daysLeftPresentation(final int daysLeft) {
		if (daysLeft < 0 && this.serverData.expired) {
			...
			if (showMonths && showDays) {
				return Component.translatable("mco.configure.world.subscription.remaining.months.days", new Object[]{months, days}).withStyle(ChatFormatting.GRAY);
			} else if (showMonths) {
				return Component.translatable("mco.configure.world.subscription.remaining.months", new Object[]{months}).withStyle(ChatFormatting.GRAY);
			} else {
				return showDays
					? Component.translatable("mco.configure.world.subscription.remaining.days", new Object[]{days}).withStyle(ChatFormatting.GRAY)
					: Component.empty();
			}
		}
	}
	...
	public void updateData(final RealmsServer serverData) {
		...
		if (snapshotWorld) {
			this.subscriptionInfo.setMessage(Component.translatable("mco.snapshot.subscription.info", new Object[]{serverData.parentWorldName}));
		} else {
