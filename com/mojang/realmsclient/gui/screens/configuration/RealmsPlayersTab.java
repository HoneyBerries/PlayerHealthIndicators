...
class RealmsPlayersTab extends GridLayoutTab implements RealmsConfigurationTab {
	...
	RealmsPlayersTab(final RealmsConfigureWorldScreen configurationScreen, final Minecraft minecraft, final RealmsServer serverData) {
		...
		helper.addChild(
			Button.builder(
					Component.translatable("mco.configure.world.buttons.invite"), button -> minecraft.setScreen(new RealmsInviteScreen(configurationScreen, serverData))
				)
				.build(),
	...
	private class HeaderEntry extends RealmsPlayersTab.Entry {
		...
		public HeaderEntry() {
			...
			Component invitedText = Component.translatable("mco.configure.world.invited.number", new Object[]{""}).withStyle(ChatFormatting.UNDERLINE);
		...
		public void extractContent(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY, final boolean hovered, final float a) {
			...
			if (!numberOfInvites.equals(this.cachedNumberOfInvites)) {
				...
				Component invitedComponent = Component.translatable("mco.configure.world.invited.number", new Object[]{numberOfInvites})
	...
	private class PlayerEntry extends RealmsPlayersTab.Entry {
		...
		public PlayerEntry(final PlayerInfo playerInfo) {
			...
			this.removeButton = SpriteIconButton.builder(REMOVE_TEXT, button -> this.uninvite(index), false)
				...
				.narration(
					defaultNarrationSupplier -> CommonComponents.joinForNarration(
						new Component[]{Component.translatable("mco.invited.player.narration", new Object[]{playerInfo.name}), (Component)defaultNarrationSupplier.get()}
					)
				)
				.build();
		...
		private void op(final int index) {
			...
			RealmsUtil.supplyAsync(
					client -> client.op(RealmsPlayersTab.this.serverData.id, selectedInvite), e -> RealmsPlayersTab.LOGGER.error("Couldn't op the user", (Throwable)e)
				)
				.thenAcceptAsync(ops -> {
					this.updateOps(ops);
					this.updateOpButtons();
		...
		private void deop(final int index) {
			UUID selectedInvite = ((PlayerInfo)RealmsPlayersTab.this.serverData.players.get(index)).uuid;
			RealmsUtil.supplyAsync(
					client -> client.deop(RealmsPlayersTab.this.serverData.id, selectedInvite), e -> RealmsPlayersTab.LOGGER.error("Couldn't deop the user", (Throwable)e)
				)
				.thenAcceptAsync(ops -> {
					this.updateOps(ops);
					this.updateOpButtons();
