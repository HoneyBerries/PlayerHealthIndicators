...
public class RealmsUploadScreen extends RealmsScreen implements RealmsWorldUploadStatusTracker {
	...
	private void extractProgressBar(final GuiGraphicsExtractor graphics) {
		...
		graphics.centeredText(this.font, Component.translatable("mco.upload.percent", new Object[]{this.progress}), this.width / 2, 84, -1);
	}
	...
	private Component createProgressNarrationMessage() {
		...
		if (this.progress != null) {
			elements.add(Component.translatable("mco.upload.percent", new Object[]{this.progress}));
		}
			...
			newUpload.packAndUpload().handleAsync((result, exception) -> {
				if (exception != null) {
					...
					if (exception instanceof RealmsUploadException e) {
						...
						this.status = Component.translatable("mco.upload.failed", new Object[]{exception.getMessage()});
					}
				} else {
