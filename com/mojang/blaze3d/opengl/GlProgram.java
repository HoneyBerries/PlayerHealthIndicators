...
public class GlProgram implements AutoCloseable {
	...
	public static final Set<String> BUILT_IN_UNIFORMS = Sets.<String>newHashSet("Projection", "Lighting", "Fog", "Globals");
		...
		if (programId <= 0) {
			...
			if (linkStatus != 0 && !linkMessage.contains("Failed for unknown reason")) {
				if (!linkMessage.isEmpty()) {
					LOGGER.info("Info log when linking program containing VS {} and FS {}. Log output: {}", vertexShader.getId(), fragmentShader.getId(), linkMessage);
				}
			...
			Object var10000_1 = switch (uniformDescription.type()) {
				...
				case TEXEL_BUFFER -> {
					...
					if (location == -1) {
						...
						yield new Uniform.Utb(location, samplerIndex, (TextureFormat)Objects.requireNonNull(uniformDescription.textureFormat()));
					}
				}
			};
