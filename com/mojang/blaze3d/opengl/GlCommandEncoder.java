...
class GlCommandEncoder implements CommandEncoderBackend {
	...
	protected <T> void executeDrawMultiple(
		...
		@Nullable VertexFormat.IndexType defaultIndexType,
	...
	protected void executeDraw(
		...
		final int drawCount,
		@Nullable final VertexFormat.IndexType indexType,
			...
			if (instanceCount > 1) {
				if (baseVertex > 0) {
					GL32.glDrawElementsInstancedBaseVertex(
						GlConst.toGl(pipeline.info().getVertexFormatMode()), drawCount, GlConst.toGl(indexType), (long)firstIndex * indexType.bytes, instanceCount, baseVertex
					);
				} else {
				...
				GlStateManager._drawElements(GlConst.toGl(pipeline.info().getVertexFormatMode()), drawCount, GlConst.toGl(indexType), (long)firstIndex * indexType.bytes);
			}
	...
	private boolean trySetup(final GlRenderPass renderPass, final Collection<String> dynamicUniforms) {
		...
		for (Entry<String, Uniform> entryx : glProgram.getUniforms().entrySet()) {
			...
			while (true) {
				switch (SwitchBootstraps.typeSwitch<"typeSwitch",Uniform.Ubo,Uniform.Utb,Uniform.Sampler>(var10, var11)) {
					case 0:
							...
							if (true) {
								...
								if (true) {
									if (differentProgram || isDirty) {
										GlStateManager._glUniform1i(var43, var43);
									}

									GlStateManager._activeTexture(33984 + var43);
