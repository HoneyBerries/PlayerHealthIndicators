...
public class PlayerGlyphProvider {
	...
	private final LoadingCache<PlayerSprite, GlyphSource> wrapperCache = CacheBuilder.newBuilder()
		...
		.build(
			new CacheLoader<PlayerSprite, GlyphSource>() {
				{
					Objects.requireNonNull(PlayerGlyphProvider.this);
				}

				public GlyphSource load(final PlayerSprite playerInfo) {
	...
	public GlyphSource sourceForPlayer(final PlayerSprite playerInfo) {
	...
	private record Instance(
		...
		public void renderSprite(
			final Matrix4fc pose, final VertexConsumer buffer, final int packedLightCoords, final float offsetX, final float offsetY, final float z, final int color
		) {
