					...
					waypoint -> {
						if (!(Boolean)waypoint.id().left().map(uuid -> uuid.equals(cameraEntity.getUUID())).orElse(false)) {
							...
							if (!(angle <= -60.0) && !(angle > 60.0)) {
								...
								int color = (Integer)icon.color
									.orElseGet(
										() -> waypoint.id()
											.map(uuid -> ARGB.setBrightness(ARGB.color(255, uuid.hashCode()), 0.9F), name -> ARGB.setBrightness(ARGB.color(255, name.hashCode()), 0.9F))
									);
								...
								PitchDirection pitchDirection = waypoint.pitchDirectionToCamera(level, this.minecraft.gameRenderer, partialTickSupplier);
