...
public class Gui {
			...
			if (player.isScoping()) {
				...
				for (EquipmentSlot slot : EquipmentSlot.values()) {
					...
					if (equippable != null && equippable.slot() == slot && equippable.cameraOverlay().isPresent()) {
						this.extractTextureOverlay(graphics, ((Identifier)equippable.cameraOverlay().get()).withPath(p -> "textures/" + p + ".png"), 1.0F);
					}
				}
			}
		}
	...
	private void displayScoreboardSidebar(final GuiGraphicsExtractor graphics, final Objective objective) {
		...
		DisplayEntry[] entriesToDisplay = (DisplayEntry[])scoreboard.listPlayerScores(objective)
			.stream()
	...
	private void extractHeart(
		final GuiGraphicsExtractor graphics, final Gui.HeartType type, final int xo, final int yo, final boolean isHardcore, final boolean blinks, final boolean half
	) {
	...
	private void extractVignette(final GuiGraphicsExtractor graphics, @Nullable final Entity camera) {
		...
		graphics.blit(
			RenderPipelines.VIGNETTE, VIGNETTE_LOCATION, 0, 0, 0.0F, 0.0F, graphics.guiWidth(), graphics.guiHeight(), graphics.guiWidth(), graphics.guiHeight(), color
		);
	}
	...
	public void setNowPlaying(final Component string) {
		Component message = Component.translatable("record.nowPlaying", new Object[]{string});
