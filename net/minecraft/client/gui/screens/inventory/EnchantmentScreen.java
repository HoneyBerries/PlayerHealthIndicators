...
public class EnchantmentScreen extends AbstractContainerScreen<EnchantmentMenu> {
	...
	public void extractRenderState(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY, final float ignored) {
		...
		for (int i = 0; i < 3; i++) {
			...
			Optional<Reference<Enchantment>> enchant = this.minecraft.level.registryAccess().lookupOrThrow(Registries.ENCHANTMENT).get(this.menu.enchantClue[i]);
			if (!enchant.isEmpty()) {
				...
				if (this.isHovering(60, 14 + 19 * i, 108, 17, mouseX, mouseY) && minLevel > 0 && enchantLevel >= 0) {
					...
					texts.add(
						Component.translatable("container.enchant.clue", new Object[]{Enchantment.getFullname((Holder)enchant.get(), enchantLevel)})
							.withStyle(ChatFormatting.WHITE)
					);
					if (!infiniteMaterials) {
						texts.add(CommonComponents.EMPTY);
						if (this.minecraft.player.experienceLevel < minLevel) {
							texts.add(Component.translatable("container.enchant.level.requirement", new Object[]{this.menu.costs[i]}).withStyle(ChatFormatting.RED));
						} else {
							MutableComponent lapisCost;
							if (cost == 1) {
								lapisCost = Component.translatable("container.enchant.lapis.one");
							} else {
								lapisCost = Component.translatable("container.enchant.lapis.many", new Object[]{cost});
							}
							...
							if (cost == 1) {
								...
								levelCost = Component.translatable("container.enchant.level.many", new Object[]{cost});
							}
