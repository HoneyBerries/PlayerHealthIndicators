...
public class CreativeModeInventoryScreen
	...
	private void tryRefreshInvalidatedTabs(final FeatureFlagSet enabledFeatures, final boolean hasPermissions, final Provider holders) {
		...
		if (this.tryRebuildTabContents(connection != null ? connection.searchTrees() : null, enabledFeatures, hasPermissions, holders)) {
			for (CreativeModeTab tab : CreativeModeTabs.allTabs()) {
				...
				if (tab == selectedTab) {
					if (tab.getType() == Type.CATEGORY && displayList.isEmpty()) {
	...
	private boolean tryRebuildTabContents(
		@Nullable final SessionSearchTrees searchTrees, final FeatureFlagSet enabledFeatures, final boolean hasPermissions, final Provider holders
	) {
	...
	protected void slotClicked(@Nullable final Slot slot, final int slotId, final int buttonNum, ContainerInput containerInput) {
		...
		if (containerInput != ContainerInput.THROW || this.minecraft.player.canDropItems()) {
			...
			if (slot == null && selectedTab.getType() != Type.INVENTORY && containerInput != ContainerInput.QUICK_CRAFT) {
	...
	public boolean preeditUpdated(@Nullable final PreeditEvent event) {
		if (this.ignoreTextInput) {
			...
			return selectedTab.getType() != Type.SEARCH ? false : this.searchBox.preeditUpdated(event);
		}
	}
	...
	private void updateVisibleTags(final String searchTerm) {
		...
		BuiltInRegistries.ITEM.getTags().map(Named::key).filter(tag -> matcher.test(tag.location())).forEach(this.visibleTags::add);
	}
	...
	public boolean mouseClicked(final MouseButtonEvent event, final boolean doubleClick) {
		if (event.button() == 0) {
			...
			if (selectedTab.getType() != Type.INVENTORY && this.insideScrollbar(event.x(), event.y())) {
	...
	private void selectTab(final CreativeModeTab tab) {
				...
				if (hotbar.isEmpty()) {
					for (int i = 0; i < 9; i++) {
						if (i == hotbarIndex) {
							...
							placeholder.set(DataComponents.ITEM_NAME, Component.translatable("inventory.hotbarInfo", new Object[]{activatorKeyMessage, translatedKeyMessage}));
		...
		if (selectedTab.getType() == Type.INVENTORY) {
			...
			for (int ix = 0; ix < invMenu.slots.size(); ix++) {
				...
				Slot slot = new CreativeModeInventoryScreen.SlotWrapper((Slot)invMenu.slots.get(ix), ix, x, y);
				this.menu.slots.add(slot);
			}

			this.destroyItemSlot = new Slot(CONTAINER, 0, 173, 112);
			this.menu.slots.add(this.destroyItemSlot);
		} else if (oldTab.getType() == Type.INVENTORY) {
			this.menu.slots.clear();
	...
	public List<Component> getTooltipFromContainerItem(final ItemStack itemStack) {
		...
		boolean isSingleCategoryTab = selectedTab.getType() == Type.CATEGORY;
		boolean isSearchTab = selectedTab.getType() == Type.SEARCH;
		Default originalTooltipStyle = this.minecraft.options.advancedItemTooltips ? Default.ADVANCED : Default.NORMAL;
		TooltipFlag tooltipStyle = isCreativeSlot ? originalTooltipStyle.asCreative() : originalTooltipStyle;
		List<Component> originalLines = itemStack.getTooltipLines(TooltipContext.of(this.minecraft.level), this.minecraft.player, tooltipStyle);
		if (originalLines.isEmpty()) {
			...
			for (CreativeModeTab tab : CreativeModeTabs.tabs()) {
				if (tab.getType() != Type.SEARCH && tab.contains(itemStack)) {
	...
	public static void handleHotbarLoadOrSave(final Minecraft minecraft, final int index, final boolean isLoadPressed, final boolean isSavePressed) {
		...
		if (isLoadPressed) {
			...
			Component message = Component.translatable("inventory.hotbarSaved", new Object[]{activatorKeyMessage, translatedKeyMessage});
