...
public class MerchantScreen extends AbstractContainerScreen<MerchantMenu> {
	...
	protected void extractLabels(final GuiGraphicsExtractor graphics, final int xm, final int ym) {
		...
		if (traderLevel > 0 && traderLevel <= 5 && this.menu.showProgressBar()) {
			Component titleAndLevel = Component.translatable("merchant.title", new Object[]{this.title, Component.translatable("merchant.level." + traderLevel)});
	...
	public boolean mouseClicked(final MouseButtonEvent event, final boolean doubleClick) {
		int xo = (this.width - this.imageWidth) / 2;
		int yo = (this.height - this.imageHeight) / 2;
		if (this.canScroll(this.menu.getOffers().size()) && event.x() > xo + 94 && event.x() < xo + 94 + 6 && event.y() > yo + 18 && event.y() <= yo + 18 + 139 + 1) {
	...
	private class TradeOfferButton extends Button.Plain {
		...
		public void extractToolTip(final GuiGraphicsExtractor graphics, final int xm, final int ym) {
			if (this.isHovered && MerchantScreen.this.menu.getOffers().size() > this.index + MerchantScreen.this.scrollOff) {
				if (xm < this.getX() + 20) {
					ItemStack item = ((MerchantOffer)MerchantScreen.this.menu.getOffers().get(this.index + MerchantScreen.this.scrollOff)).getCostA();
					graphics.setTooltipForNextFrame(MerchantScreen.this.font, item, xm, ym);
				} else if (xm < this.getX() + 50 && xm > this.getX() + 30) {
					ItemStack item = ((MerchantOffer)MerchantScreen.this.menu.getOffers().get(this.index + MerchantScreen.this.scrollOff)).getCostB();
					if (!item.isEmpty()) {
						graphics.setTooltipForNextFrame(MerchantScreen.this.font, item, xm, ym);
					}
				} else if (xm > this.getX() + 65) {
					ItemStack item = ((MerchantOffer)MerchantScreen.this.menu.getOffers().get(this.index + MerchantScreen.this.scrollOff)).getResult();
					graphics.setTooltipForNextFrame(MerchantScreen.this.font, item, xm, ym);
				}
			}
		}
