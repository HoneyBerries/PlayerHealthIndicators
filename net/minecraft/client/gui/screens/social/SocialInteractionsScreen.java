...
public class SocialInteractionsScreen extends Screen {
	...
	private void showPage(final SocialInteractionsScreen.Page page) {
		...
		switch (page) {
			...
			case BLOCKED:
				...
				Set<UUID> blockedPlayers = (Set<UUID>)this.minecraft
					.player
					.connection
					.getOnlinePlayerIds()
					.stream()
	...
	private void updateServerLabel(final Minecraft minecraft) {
		...
		if (this.playerCount != playerCount) {
			...
			if (playerCount > 1) {
				this.serverLabel = Component.translatable("gui.socialInteractions.server_label.multiple", new Object[]{serverName, playerCount});
			} else {
				this.serverLabel = Component.translatable("gui.socialInteractions.server_label.single", new Object[]{serverName, playerCount});
			}
