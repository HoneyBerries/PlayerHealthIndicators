...
import net.minecraft.world.level.gamerules.GameRuleMap.Builder;
...
public class CreateWorldScreen extends Screen {
	...
	public static void openFresh(final Minecraft minecraft, final Runnable onClose, final CreateWorldCallback createWorld) {
		...
		Function<DataLoadContext, WorldGenSettings> worldGenSettings = context -> new WorldGenSettings(
	...
	public static void testWorld(final Minecraft minecraft, final Runnable onClose) {
		WorldCreationContextMapper worldCreationContext = (managers, registries, cookie) -> new WorldCreationContext(
			...
			new InitialWorldCreationOptions(
				...
				new Builder().set(GameRules.ADVANCE_TIME, false).set(GameRules.ADVANCE_WEATHER, false).set(GameRules.SPAWN_MOBS, false).build(),
	...
	private static void openCreateWorldScreen(
		...
		final Function<DataLoadContext, WorldGenSettings> worldGenSettings,
		...
		InitConfig loadConfig = createDefaultLoadConfig(vanillaOnlyPackRepository, dataConfig);
		CompletableFuture<WorldCreationContext> loadResult = WorldLoader.load(
			loadConfig,
			context -> new DataLoadOutput(
				new DataPackReloadCookie((WorldGenSettings)worldGenSettings.apply(context), context.dataConfiguration()), context.datapackDimensions()
			),
		...
		minecraft.setScreen(
			new CreateWorldScreen(minecraft, onClose, (WorldCreationContext)loadResult.join(), Optional.of(worldPreset), OptionalLong.empty(), createWorld)
		);
	}
	...
	private void onCreate() {
		...
		Complete finalDimensions = worldDimensions.bake(context.datapackDimensions());
		LayeredRegistryAccess<RegistryLayer> finalLayers = context.worldgenRegistries()
			.replaceFrom(RegistryLayer.DIMENSIONS, new Frozen[]{finalDimensions.dimensionsRegistryAccess()});
		...
		boolean isDebug = finalDimensions.specialWorldProperty() == SpecialWorldProperty.DEBUG;
		...
		WorldDataAndGenSettings worldDataAndGenSettings = new WorldDataAndGenSettings(worldData, worldGenSettings);
	...
	private void createWorldAndCleanup(
		final LayeredRegistryAccess<RegistryLayer> finalLayers, final WorldDataAndGenSettings worldDataAndGenSettings, final Optional<GameRules> gameRules
	) {
	...
	private boolean createNewWorld(
		...
		Optional<LevelStorageAccess> newWorldAccess = createNewWorldDirectory(this.minecraft, worldFolder, this.tempDataPackDir);
		if (newWorldAccess.isEmpty()) {
			...
			this.minecraft
				...
				.createLevelFromExistingSettings((LevelStorageAccess)newWorldAccess.get(), context.dataPackResources(), finalLayers, worldDataAndGenSettings, gameRules);
			return true;
		}
	}
	...
	private LevelSettings createLevelSettings(final boolean isDebug) {
		...
		return isDebug
			? new LevelSettings(name, GameType.SPECTATOR, new DifficultySettings(Difficulty.PEACEFUL, false, false), true, WorldDataConfiguration.DEFAULT)
			: new LevelSettings(
				name,
				this.uiState.getGameMode().gameType,
				new DifficultySettings(this.uiState.getDifficulty(), this.uiState.isHardcore(), false),
	...
	private void openExperimentsScreen(final WorldDataConfiguration dataConfiguration) {
		...
		if (settings != null) {
			this.minecraft
				.setScreen(
					new ExperimentsScreen(this, settings.getSecond(), packRepository -> this.tryApplyNewDataPacks(packRepository, false, this::openExperimentsScreen))
				);
		}
	}
	...
	private void tryApplyNewDataPacks(final PackRepository packRepository, final boolean isDataPackScreen, final Consumer<WorldDataConfiguration> onAbort) {
		...
		List<String> newDisabled = (List<String>)packRepository.getAvailableIds()
			.stream()
			.filter(id -> !newEnabled.contains(id))
	...
	private void applyNewPackConfig(final PackRepository packRepository, final WorldDataConfiguration newConfig, final Consumer<WorldDataConfiguration> onAbort) {
		...
		InitConfig config = createDefaultLoadConfig(packRepository, newConfig);
		WorldLoader.load(
				config,
				context -> {
					if (context.datapackWorldgen().lookupOrThrow(Registries.WORLD_PRESET).listElements().findAny().isEmpty()) {
						...
						return new DataLoadOutput(new DataPackReloadCookie(settings, context.dataConfiguration()), context.datapackDimensions());
					}
				},
				(resources, managers, registries, cookie) -> {
					resources.close();
	...
	private static InitConfig createDefaultLoadConfig(final PackRepository packRepository, final WorldDataConfiguration config) {
		PackConfig packConfig = new PackConfig(packRepository, config, false, true);
		return new InitConfig(packConfig, CommandSelection.INTEGRATED, LevelBasedPermissionSet.GAMEMASTER);
	}
	...
	private static Optional<LevelStorageAccess> createNewWorldDirectory(final Minecraft minecraft, final String worldFolder, @Nullable final Path tempDataPackDir) {
		try {
			LevelStorageAccess access = minecraft.getLevelSource().createAccess(worldFolder);
			...
			CreateWorldScreen.this.uiState
				.addListener(
					uiState -> this.nameEdit
						.setTooltip(
							Tooltip.create(
								Component.translatable("selectWorld.targetFolder", new Object[]{Component.literal(uiState.getTargetFolder()).withStyle(ChatFormatting.ITALIC)})
							)
						)
				);
			CreateWorldScreen.this.setInitialFocus(this.nameEdit);
			helper.addChild(
				CommonLayouts.labeledElement(CreateWorldScreen.this.font, this.nameEdit, CreateWorldScreen.NAME_LABEL), helper.newCellSettings().alignHorizontallyCenter()
			);
			CycleButton<WorldCreationUiState.SelectedGameMode> gameModeButton = helper.addChild(
				CycleButton.<WorldCreationUiState.SelectedGameMode>builder(selectedGameMode -> selectedGameMode.displayName, CreateWorldScreen.this.uiState.getGameMode())
	...
	private class MoreTab extends GridLayoutTab {
		...
		private MoreTab() {
			...
			helper.addChild(
				Button.builder(
						CreateWorldScreen.EXPERIMENTS_LABEL, b -> CreateWorldScreen.this.openExperimentsScreen(CreateWorldScreen.this.uiState.getSettings().dataConfiguration())
					)
					.width(210)
					.build()
			);
			helper.addChild(
				Button.builder(DATA_PACKS_LABEL, b -> CreateWorldScreen.this.openDataPackSelectionScreen(CreateWorldScreen.this.uiState.getSettings().dataConfiguration()))
					.width(210)
					.build()
			);
		}
			...
			CreateWorldScreen.this.minecraft
				.setScreen(
					new WorldCreationGameRulesScreen(
						CreateWorldScreen.this.uiState.getGameRules().copy(CreateWorldScreen.this.uiState.getSettings().dataConfiguration().enabledFeatures()), gameRules -> {
	...
	private class WorldTab extends GridLayoutTab {
		...
		private WorldTab() {
			...
			CycleButton<WorldCreationUiState.WorldTypeEntry> typeButton = helper.addChild(
				CycleButton.builder(WorldCreationUiState.WorldTypeEntry::describePreset, CreateWorldScreen.this.uiState.getWorldType())
					...
					.create(0, 0, 150, 20, Component.translatable("selectWorld.mapType"), (button, newPreset) -> CreateWorldScreen.this.uiState.setWorldType(newPreset))
			);
			...
			switchGridBuilder.addSwitch(GENERATE_STRUCTURES, CreateWorldScreen.this.uiState::isGenerateStructures, CreateWorldScreen.this.uiState::setGenerateStructures)
		...
		private static MutableComponent createTypeButtonNarration(final CycleButton<WorldCreationUiState.WorldTypeEntry> button) {
			return button.getValue().isAmplified()
				? CommonComponents.joinForNarration(new Component[]{button.createDefaultNarrationMessage(), AMPLIFIED_HELP_TEXT})
