...
import net.minecraft.util.worldupdate.UpgradeProgress.FileFixStats;
import net.minecraft.util.worldupdate.UpgradeProgress.Type;
...
public class FileFixerProgressScreen extends Screen {
	...
	private void extractProgress(final GuiGraphicsExtractor graphics, final int xCenter, final int textTop) {
		FileFixStats typeFileStats = this.upgradeProgress.getTypeFileFixStats();
		FileFixStats totalFileStats = this.upgradeProgress.getTotalFileFixStats();
		FileFixStats runningFileFixerStats = this.upgradeProgress.getRunningFileFixerStats();
		int y = textTop + 9 + 3;
	...
	private void extractTypeText(final GuiGraphicsExtractor graphics, final int xCenter, final int y) {
		Type upgradeProgressType = this.upgradeProgress.getType();
	...
	private void extractTypeProgress(final GuiGraphicsExtractor graphics, final int xCenter, final int y, final float progress) {
		Component percentageText = Component.translatable("upgradeWorld.progress.percentage", new Object[]{Mth.floor(progress * 100.0F)});
	...
	private void extractFileStats(final GuiGraphicsExtractor graphics, final int xCenter, final int yStart, final int converted, final int total) {
		int lineHeight = 9 + 3;
		graphics.centeredText(this.font, Component.translatable("upgradeWorld.info.converted", new Object[]{converted}), xCenter, yStart, -6250336);
		graphics.centeredText(this.font, Component.translatable("upgradeWorld.info.total", new Object[]{total}), xCenter, yStart + lineHeight, -6250336);
	}
	...
	private void extractFileFixerCount(final GuiGraphicsExtractor graphics, final int xCenter, final int y, final int current, final int total) {
		Component percentageText = Component.translatable("upgradeWorld.info.file_fix_stage", new Object[]{current, total});
		graphics.centeredText(this.font, percentageText, xCenter, y, -6250336);
	}
}
