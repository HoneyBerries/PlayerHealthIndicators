...
public class WorldOpenFlows {
	...
	public void createFreshLevel(
		...
		final Function<Provider, WorldDimensions> dimensionsProvider,
		...
		LevelStorageAccess levelSourceAccess = this.createWorldAccess(levelId);
		if (levelSourceAccess != null) {
			...
			try {
				PackConfig packConfig = new PackConfig(packRepository, dataConfiguration, false, false);
				WorldStem worldStem = this.loadWorldDataBlocking(
					...
					context -> {
						...
						Complete completeDimensions = dimensions.bake(context.datapackDimensions().lookupOrThrow(Registries.LEVEL_STEM));
						return new DataLoadOutput(
							new WorldDataAndGenSettings(
	...
	private LevelStorageAccess createWorldAccess(final String levelId) {
		try {
	...
	public void createLevelFromExistingSettings(
		...
		PackRepository packRepository = ServerPacksSource.createPackRepository(levelSourceAccess);
		CloseableResourceManager resourceManager = (CloseableResourceManager)new PackConfig(
				packRepository, worldDataAndGenSettings.data().getDataConfiguration(), false, false
			)
			.createResourceManager()
			.getSecond();
		this.minecraft
			.doWorldLoad(levelSourceAccess, packRepository, new WorldStem(resourceManager, serverResources, registryAccess, worldDataAndGenSettings), gameRules, true);
	}

	public WorldStem loadWorldStem(
		final LevelStorageAccess worldAccess, final Dynamic<?> levelDataTag, final boolean safeMode, final PackRepository packRepository
	) throws Exception {
		PackConfig packConfig = LevelStorageSource.getPackConfig(levelDataTag, packRepository, safeMode);
		return this.loadWorldDataBlocking(
			...
			context -> {
				...
				return new DataLoadOutput(data.worldDataAndGenSettings(), data.dimensions().dimensionsRegistryAccess());
			},
			WorldStem::new
		);
	}

	public Pair<LevelSettings, WorldCreationContext> recreateWorldData(final LevelStorageAccess levelSourceAccess) throws Exception {
		PackRepository packRepository = ServerPacksSource.createPackRepository(levelSourceAccess);
		...
		if (DataFixers.getFileFixer().requiresFileFixing(dataVersion)) {
			...
			PackConfig packConfig = LevelStorageSource.getPackConfig(levelDataTag, packRepository, false);
			...
			return this.loadWorldDataBlocking(
				...
				(resources, managers, registries, loadedData) -> {
					...
					InitialWorldCreationOptions initialWorldCreationOptions = new InitialWorldCreationOptions(
						WorldCreationUiState.SelectedGameMode.SURVIVAL, (GameRuleMap)existingGameRules.result().orElse(GameRuleMap.of()), null
					);
					return Pair.of(
	...
	private <D, R> R loadWorldDataBlocking(final PackConfig packConfig, final WorldDataSupplier<D> worldDataGetter, final ResultFactory<D, R> worldDataSupplier) throws Exception {
		long start = Util.getMillis();
		InitConfig config = new InitConfig(packConfig, CommandSelection.INTEGRATED, LevelBasedPermissionSet.GAMEMASTER);
	...
	private void askForBackup(final LevelStorageAccess levelAccess, final boolean oldCustomized, final Runnable proceedCallback, final Runnable cancelCallback) {
	...
	public void openWorld(final String levelId, final Runnable onCancel) {
		this.minecraft.setScreenAndShow(new GenericMessageScreen(Component.translatable("selectWorld.data_read")));
		LevelStorageAccess worldAccess = this.createWorldAccess(levelId);
		if (worldAccess != null) {
			this.openWorldLoadLevelData(worldAccess, onCancel);
		}
	}

	private void openWorldLoadLevelData(final LevelStorageAccess worldAccess, final Runnable onCancel) {
		this.minecraft.setScreenAndShow(new GenericMessageScreen(Component.translatable("selectWorld.data_read")));
	...
	private void openWorldCheckVersionCompatibility(
		final LevelStorageAccess worldAccess, final LevelSummary summary, final Dynamic<?> levelDataTag, final Runnable onCancel
	) {
		if (!summary.isCompatible()) {
			...
			this.minecraft
				.setScreen(
					new AlertScreen(
						...
						Component.translatable("selectWorld.incompatible.description", new Object[]{summary.getWorldVersionName()})
					)
				);
		} else {
			BackupStatus backupStatus = summary.backupStatus();
			if (backupStatus.shouldBackup()) {
				...
				Component backupWarning = Component.translatable(warningKey, new Object[]{summary.getWorldVersionName(), SharedConstants.getCurrentVersion().name()});
	...
	private void createBackupAndOpenWorld(final LevelStorageAccess levelAccess, final Dynamic<?> levelDataTag, final Runnable onCancel, final boolean backup) {
	...
	private void upgradeAndOpenWorld(final LevelStorageAccess worldAccess, final Dynamic<?> levelDataTag, final Runnable onCancel) {
		Runnable cleanup = () -> {
			worldAccess.safeClose();
			onCancel.run();
		};
	...
	private Dynamic<?> tryFileFixAndReportErrors(
		final LevelStorageAccess worldAccess, final Dynamic<?> levelDataTag, final UpgradeProgress upgradeProgress, final Runnable cleanup
	) {
		try {
			...
			this.minecraft
				.execute(
					() -> this.minecraft
						.setScreenAndShow(
							new AlertScreen(
								cleanup, Component.translatable("upgradeWorld.canceled.title"), Component.translatable("upgradeWorld.canceled.message"), CommonComponents.GUI_OK, true
							)
						)
				);
			return null;
				...
				.execute(
					() -> {
						if (var9.getCause() instanceof CowFSSymlinkException) {
							this.minecraft
								.setScreenAndShow(
									new AlertScreen(cleanup, Component.translatable("upgradeWorld.symlink.title"), Component.translatable("upgradeWorld.symlink.message"))
								);
						} else {
					...
					() -> this.minecraft
						.setScreenAndShow(
							new AlertScreen(
								...
								Component.translatable("upgradeWorld.failed_cleanup.message", new Object[]{Component.literal(var10.newWorldFolderName()).withColor(-8355712)})
							)
						)
				);
			return null;
	...
	private void openWorldLoadLevelStem(final LevelStorageAccess worldAccess, final Dynamic<?> levelDataTag, final boolean safeMode, final Runnable onCancel) {
	...
	private void openWorldCheckWorldStemCompatibility(
		final LevelStorageAccess worldAccess, final WorldStem worldStem, final PackRepository packRepository, final Runnable onCancel
	) {
		WorldDataAndGenSettings worldDataAndGenSettings = worldStem.worldDataAndGenSettings();
	...
	private void openWorldCheckDiskSpace(
		final LevelStorageAccess worldAccess,
	...
	private void openWorldDoLoad(final LevelStorageAccess worldAccess, final WorldStem worldStem, final PackRepository packRepository) {
		this.minecraft.doWorldLoad(worldAccess, packRepository, worldStem, Optional.empty(), false);
	}

	private CompletableFuture<Void> loadBundledResourcePack(final DownloadedPackSource packSource, final LevelStorageAccess levelSourceAccess) {
