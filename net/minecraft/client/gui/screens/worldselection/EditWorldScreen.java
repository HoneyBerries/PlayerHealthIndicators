...
public class EditWorldScreen extends Screen {
	...
	public static EditWorldScreen create(final Minecraft minecraft, final LevelStorageAccess levelAccess, final BooleanConsumer callback) throws IOException {
	...
	private EditWorldScreen(final Minecraft minecraft, final LevelStorageAccess levelAccess, final String name, final BooleanConsumer callback) {
						...
						button -> minecraft.setScreen(
							new BackupConfirmScreen(
								...
								(backup, eraseCache) -> conditionallyMakeBackupAndShowToast(backup, levelAccess)
									.thenAcceptAsync(
										var4x -> minecraft.setScreen(OptimizeWorldScreen.create(minecraft, this.callback, minecraft.getFixerUpper(), levelAccess, eraseCache)), minecraft
									),
								OPTIMIZE_TITLE,
	...
	public static CompletableFuture<Boolean> conditionallyMakeBackupAndShowToast(final boolean createBackup, final LevelStorageAccess access) {
		return createBackup ? makeBackupAndShowToast(access) : CompletableFuture.completedFuture(false);
	}

	public static CompletableFuture<Boolean> makeBackupAndShowToast(final LevelStorageAccess access) {
		Minecraft minecraft = Minecraft.getInstance();
		minecraft.setScreenAndShow(
			GenericWaitingScreen.createWaitingWithoutButton(
				Component.translatable("selectWorld.waitingForBackup.title"), Component.translatable("selectWorld.waitingForBackup.message").withStyle(ChatFormatting.GRAY)
			)
		);
		return CompletableFuture.supplyAsync(() -> {
			...
			Component title = Component.translatable("selectWorld.edit.backupCreated", new Object[]{access.getLevelId()});
			Component message = Component.translatable("selectWorld.edit.backupSize", new Object[]{Mth.ceil(size.longValue() / 1048576.0)});
