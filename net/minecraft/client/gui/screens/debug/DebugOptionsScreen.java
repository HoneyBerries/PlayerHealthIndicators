...
public class DebugOptionsScreen extends Screen {
	...
	private class CategoryEntry extends DebugOptionsScreen.AbstractOptionEntry {
		...
		public void extractContent(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY, final boolean hovered, final float a) {
			graphics.centeredText(DebugOptionsScreen.this.minecraft.font, this.category, this.getContentX() + this.getContentWidth() / 2, this.getContentY() + 5, -1);
		}
	...
	private class OptionEntry extends DebugOptionsScreen.AbstractOptionEntry {
		...
		private MutableComponent narrateButton(final CycleButton<Boolean> booleanCycleButton) {
			...
			MutableComponent current = Component.translatable("debug.entry.currently." + status.getSerializedName(), new Object[]{this.name});
	...
	public class OptionList extends ContainerObjectSelectionList<DebugOptionsScreen.AbstractOptionEntry> {
		private static final Comparator<java.util.Map.Entry<Identifier, DebugScreenEntry>> COMPARATOR = (o1, o2) -> {
			int byCategory = FloatComparators.NATURAL_COMPARATOR
				.compare(((DebugScreenEntry)o1.getValue()).category().sortKey(), ((DebugScreenEntry)o2.getValue()).category().sortKey());
		...
		public void updateSearch(final String value) {
			...
			for (java.util.Map.Entry<Identifier, DebugScreenEntry> entry : all) {
