...
public class RecoverWorldDataScreen extends Screen {
	...
	public RecoverWorldDataScreen(final Minecraft minecraft, final BooleanConsumer callback, final LevelStorageAccess storageAccess) {
		super(TITLE);
		this.callback = callback;
		this.message = Component.translatable("recover_world.message", new Object[]{Component.literal(storageAccess.getLevelId()).withStyle(ChatFormatting.GRAY)});
	...
	private Component buildInfo(final LevelStorageAccess access, final boolean fallback, @Nullable final Exception exception) {
		if (fallback && exception instanceof FileNotFoundException) {
			...
			component.append(Component.translatable("recover_world.state_entry", new Object[]{time.withStyle(ChatFormatting.GRAY)}));
			if (exception == null) {
	...
	@Nullable
	private Exception collectIssue(final LevelStorageAccess access, final boolean useFallback) {
		try {
