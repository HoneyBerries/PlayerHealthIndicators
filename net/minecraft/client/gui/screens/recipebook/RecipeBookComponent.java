...
public abstract class RecipeBookComponent<T extends RecipeBookMenu> implements GuiEventListener, Renderable, NarratableEntry {
	...
	private void initVisuals() {
		...
		if (this.selectedTab != null) {
			this.selectedTab = (RecipeBookTabButton)this.tabButtons
				.stream()
				.filter(o -> o.getCategory().equals(this.selectedTab.getCategory()))
				.findFirst()
				.orElse(null);
		}
	...
	private void updateCollections(final boolean resetPage, final boolean isFiltering) {
		...
		List<RecipeCollection> collection = Lists.<RecipeCollection>newArrayList(tabCollection);
	...
	public void updateNarration(final NarrationElementOutput output) {
		List<NarratableEntry> narratableEntries = Lists.<NarratableEntry>newArrayList();
