	...
	protected void init() {
		...
		this.chatAbilities
			...
			.forEach(
				restriction -> {
					body.addChild(
						FocusableTextWidget.builder(ComponentUtils.mergeStyles(restriction.display(), Style.EMPTY.withBold(true).withColor(ChatFormatting.RED)), this.font)
							.maxWidth(250)
					...
					restriction.action()
						.ifPresent(
							action -> body.addChild(Button.builder(action.title(), var2x -> action.runnable().accept(this.minecraft, this.previousScreen)).width(200).build())
						);
				}
			);
