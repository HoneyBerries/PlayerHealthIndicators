...
public class ServerSelectionList extends ObjectSelectionList<ServerSelectionList.Entry> {
	...
	public void updateNetworkServers(final List<LanServer> servers) {
		...
		for (int i = this.networkServers.size() - newServerCount; i < this.networkServers.size(); i++) {
			ServerSelectionList.NetworkServerEntry newServer = (ServerSelectionList.NetworkServerEntry)this.networkServers.get(i);
			...
			if (rowBottom >= this.getY() && rowTop <= this.getBottom()) {
				this.minecraft.getNarrator().saySystemQueued(Component.translatable("multiplayer.lan.server_found", new Object[]{newServer.getServerNarration()}));
			}
		}
	}
	...
	public class OnlineServerEntry extends ServerSelectionList.Entry implements SelectableEntry {
		...
		public void extractContent(final GuiGraphicsExtractor graphics, final int mouseX, final int mouseY, final boolean hovered, final float a) {
			...
			for (int i = 0; i < Math.min(lines.size(), 2); i++) {
				graphics.text(this.minecraft.font, (FormattedCharSequence)lines.get(i), this.getContentX() + 32 + 3, this.getContentY() + 12 + 9 * i, -8355712);
			}
			...
			if (this.minecraft.options.touchscreen().get() || hovered) {
				...
				if (index > 0) {
					if (this.mouseOverTopLeftQuarter(relX, relY, 32)) {
						graphics.blitSprite(RenderPipelines.GUI_TEXTURED, ServerSelectionList.MOVE_UP_HIGHLIGHTED_SPRITE, this.getContentX(), this.getContentY(), 32, 32);
						ServerSelectionList.this.handleCursor(graphics);
					} else {
				...
				if (index < this.screen.getServers().size() - 1) {
					if (this.mouseOverBottomLeftQuarter(relX, relY, 32)) {
						graphics.blitSprite(RenderPipelines.GUI_TEXTURED, ServerSelectionList.MOVE_DOWN_HIGHLIGHTED_SPRITE, this.getContentX(), this.getContentY(), 32, 32);
						ServerSelectionList.this.handleCursor(graphics);
					} else {
		...
		public Component getNarration() {
			...
			narrationComponent.append(Component.translatable("narrator.select", new Object[]{this.serverData.name}));
			...
			switch (this.serverData.state()) {
				...
				case INCOMPATIBLE:
					...
					narrationComponent.append(CommonComponents.NARRATION_SEPARATOR);
					narrationComponent.append(Component.translatable("multiplayer.status.version.narration", new Object[]{this.serverData.version}));
					narrationComponent.append(CommonComponents.NARRATION_SEPARATOR);
					narrationComponent.append(Component.translatable("multiplayer.status.motd.narration", new Object[]{this.serverData.motd}));
					break;
				case UNREACHABLE:
				...
				default:
					...
					narrationComponent.append(CommonComponents.NARRATION_SEPARATOR);
					narrationComponent.append(Component.translatable("multiplayer.status.ping.narration", new Object[]{this.serverData.ping}));
					narrationComponent.append(CommonComponents.NARRATION_SEPARATOR);
					narrationComponent.append(Component.translatable("multiplayer.status.motd.narration", new Object[]{this.serverData.motd}));
