...
public class BanNoticeScreens {
	...
	public static ConfirmLinkScreen create(final BooleanConsumer callback, final BanDetails multiplayerBanned) {
		return new ConfirmLinkScreen(
			callback, getBannedTitle(multiplayerBanned), getBannedScreenText(multiplayerBanned), CommonLinks.SUSPENSION_HELP, CommonComponents.GUI_ACKNOWLEDGE, true
		);
	}
	...
	public static ConfirmLinkScreen createNameBan(final String name, final Runnable onClose) {
		...
		return new ConfirmLinkScreen(
			...
			Component.translatable(
				...
				new Object[]{Component.literal(name).withStyle(ChatFormatting.YELLOW), Component.translationArg(CommonLinks.SUSPENSION_HELP)}
			),
			uri,
	...
	private static Component getBannedScreenText(final BanDetails multiplayerBanned) {
		return Component.translatable(
			...
			new Object[]{getBanReasonText(multiplayerBanned), getBanStatusText(multiplayerBanned), Component.translationArg(CommonLinks.SUSPENSION_HELP)}
		);
	}
	...
	private static Component getBanReasonText(final BanDetails multiplayerBanned) {
		...
		if (StringUtils.isNumeric(reasonString)) {
			...
			if (reason != null) {
				...
				reasonText = Component.translatable("gui.banned.description.reason_id_message", new Object[]{reasonId, reasonMessage}).withStyle(ChatFormatting.BOLD);
			} else {
				reasonText = Component.translatable("gui.banned.description.reason_id", new Object[]{reasonId}).withStyle(ChatFormatting.BOLD);
			}

			return Component.translatable("gui.banned.description.reason", new Object[]{reasonText});
		} else {
	...
	private static Component getBanStatusText(final BanDetails multiplayerBanned) {
		if (isTemporaryBan(multiplayerBanned)) {
			...
			return Component.translatable(
				...
				new Object[]{Component.translatable("gui.banned.description.temporary.duration", new Object[]{banDurationText}).withStyle(ChatFormatting.BOLD)}
			);
		} else {
