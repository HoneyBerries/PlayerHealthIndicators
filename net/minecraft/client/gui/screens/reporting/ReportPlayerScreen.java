...
public class ReportPlayerScreen extends Screen {
	...
	protected void init() {
		...
		Button chatButton = this.layout
			.addChild(Button.builder(REPORT_CHAT, b -> this.minecraft.setScreen(new ChatReportScreen(this.lastScreen, this.context, this.player.getPlayerId()))).build());
		if (this.chatDisabledOrBlocked) {
			...
			chatButton.active = false;
			chatButton.setTooltip(Tooltip.create(Component.translatable("gui.socialInteractions.tooltip.report.no_messages", new Object[]{this.player.getPlayerName()})));
		}

		this.layout
			.addChild(
				Button.builder(
						REPORT_SKIN, b -> this.minecraft.setScreen(new SkinReportScreen(this.lastScreen, this.context, this.player.getPlayerId(), this.player.getSkinGetter()))
					)
					.build()
			);
		this.layout
			.addChild(
				Button.builder(
						REPORT_NAME, b -> this.minecraft.setScreen(new NameReportScreen(this.lastScreen, this.context, this.player.getPlayerId(), this.player.getPlayerName()))
					)
					.build()
			);
