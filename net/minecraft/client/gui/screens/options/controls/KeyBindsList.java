	...
	public class KeyEntry extends KeyBindsList.Entry {
			...
			this.changeButton = Button.builder(name, button -> {
				...
				.createNarration(
					defaultNarrationSupplier -> key.isUnbound()
						? Component.translatable("narrator.controls.unbound", new Object[]{name})
						: Component.translatable("narrator.controls.bound", new Object[]{name, defaultNarrationSupplier.get()})
				)
				.build();
			this.resetButton = Button.builder(RESET_BUTTON_TITLE, button -> {
				key.setKey(key.getDefaultKey());
				KeyBindsList.this.resetMappingAndUpdateButtons();
			}).bounds(0, 0, 50, 20).createNarration(defaultNarrationSupplier -> Component.translatable("narrator.controls.reset", new Object[]{name})).build();
			this.refreshEntry();
		}
		...
		protected void refreshEntry() {
			...
			if (this.hasCollision) {
				...
				this.changeButton.setTooltip(Tooltip.create(Component.translatable("controls.keybinds.duplicateKeybinds", new Object[]{tooltip})));
			} else {
