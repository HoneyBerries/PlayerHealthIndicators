				...
				if (monitor == null) {
					...
					return Options.genericValueLabel(
						...
						Component.translatable(
							...
							new Object[]{mode.getWidth(), mode.getHeight(), mode.getRefreshRate(), mode.getRedBits() + mode.getGreenBits() + mode.getBlueBits()}
						)
					);
				}
			},
	...
	public boolean mouseClicked(final MouseButtonEvent event, final boolean doubleClick) {
		if (super.mouseClicked(event, doubleClick)) {
			if (this.gpuWarnlistManager.isShowingWarning()) {
				List<Component> warningMessage = Lists.<Component>newArrayList(WARNING_MESSAGE, CommonComponents.NEW_LINE);
				...
				if (rendererWarnings != null) {
					...
					warningMessage.add(Component.translatable("options.graphics.warning.renderer", new Object[]{rendererWarnings}).withStyle(ChatFormatting.GRAY));
				}

				String vendorWarnings = this.gpuWarnlistManager.getVendorWarnings();
				if (vendorWarnings != null) {
					warningMessage.add(CommonComponents.NEW_LINE);
					warningMessage.add(Component.translatable("options.graphics.warning.vendor", new Object[]{vendorWarnings}).withStyle(ChatFormatting.GRAY));
				}

				String versionWarnings = this.gpuWarnlistManager.getVersionWarnings();
				if (versionWarnings != null) {
					warningMessage.add(CommonComponents.NEW_LINE);
					warningMessage.add(Component.translatable("options.graphics.warning.version", new Object[]{versionWarnings}).withStyle(ChatFormatting.GRAY));
				}
