...
public class ChatComponent {
	...
	private static final Component RESTRICTED_CHAT_MESSAGE = Component.translatable("chat_screen.restricted")
		.withStyle(Style.EMPTY.withColor(ChatFormatting.RED).withUnderlined(true).withClickEvent(new Custom(GO_TO_RESTRICTIONS_SCREEN, Optional.empty())));
	private static final Component RESTRICTED_CHAT_MESSAGE_WITH_HOVER = ComponentUtils.mergeStyles(
		RESTRICTED_CHAT_MESSAGE, Style.EMPTY.withHoverEvent(new ShowText(Component.translatable("chat_screen.restricted")))
	);
	...
	private void extractRenderState(
		...
		if (total > 0 || isRestricted) {
			...
			ChatComponent.AlphaCalculator alphaCalculator = isForeground ? ChatComponent.AlphaCalculator.FULLY_VISIBLE : ChatComponent.AlphaCalculator.timeBased(ticks);
	...
	private void addMessage(final Component contents, @Nullable final MessageSignature signature, final GuiMessageSource source, @Nullable final GuiMessageTag tag) {
