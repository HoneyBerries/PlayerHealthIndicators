...
public class SkinManager {
	...
	public Supplier<PlayerSkin> createLookup(final GameProfile profile, final boolean requireSecure) {
		...
		if (SharedConstants.DEBUG_DEFAULT_SKIN_OVERRIDE) {
			...
			Optional<PlayerSkin> currentValue = (Optional<PlayerSkin>)future.getNow(null);
	...
	private CompletableFuture<PlayerSkin> registerTextures(final UUID profileId, final MinecraftProfileTextures textures) {
		...
		CompletableFuture<Texture> skinTexture;
		...
		CompletableFuture<Texture> capeTexture = capeInfo != null ? this.capeTextures.getOrLoad(capeInfo) : CompletableFuture.completedFuture(null);
		MinecraftProfileTexture elytraInfo = textures.elytra();
		CompletableFuture<Texture> elytraTexture = elytraInfo != null ? this.elytraTextures.getOrLoad(elytraInfo) : CompletableFuture.completedFuture(null);
		return CompletableFuture.allOf(skinTexture, capeTexture, elytraTexture)
			.thenApply(
				unused -> new PlayerSkin(
					(Texture)skinTexture.join(), (Texture)capeTexture.join(), (Texture)elytraTexture.join(), model, textures.signatureState() == SignatureState.SIGNED
				)
			);
	}
	...
	private class TextureCache {
		...
		private final Map<String, CompletableFuture<Texture>> textures;
		...
		public CompletableFuture<Texture> getOrLoad(final MinecraftProfileTexture texture) {
			String hash = texture.getHash();
			CompletableFuture<Texture> future = (CompletableFuture<Texture>)this.textures.get(hash);
			if (future == null) {
		...
		private CompletableFuture<Texture> registerTexture(final MinecraftProfileTexture textureInfo) {
