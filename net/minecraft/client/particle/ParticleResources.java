...
public class ParticleResources implements PreparableReloadListener {
	...
	public CompletableFuture<Void> reload(
		...
		CompletableFuture<List<ParticleDefinition>> spriteSetsToLoad = CompletableFuture.supplyAsync(
				() -> PARTICLE_LISTER.listMatchingResources(manager), taskExecutor
			)
			.thenCompose(definitionsToScan -> {
				...
				definitionsToScan.forEach((resourceId, resource) -> {
					...
					loadTasks.add(CompletableFuture.supplyAsync(() -> new ParticleDefinition(particleId, this.loadParticleDescription(particleId, resource)), taskExecutor));
				});
				return Util.sequence(loadTasks);
			});
		CompletableFuture<SpriteLoader.Preparations> pendingSprites = ((AtlasManager.PendingStitchResults)currentReload.get(AtlasManager.PENDING_STITCH))
		...
		return CompletableFuture.allOf(spriteSetsToLoad, pendingSprites).thenCompose(preparationBarrier::wait).thenAcceptAsync(unused -> {
			...
			((List)spriteSetsToLoad.join()).forEach(p -> {
				...
				if (!spriteIds.isEmpty()) {
					...
					((ParticleResources.MutableSpriteSet)this.spriteSets.get(p.id())).rebind(contents);
				}
			});
