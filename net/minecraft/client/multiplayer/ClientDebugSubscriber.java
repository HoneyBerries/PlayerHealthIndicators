...
public class ClientDebugSubscriber {
	...
	public DebugValueAccess createDebugValueAccess(final Level level) {
		return new DebugValueAccess() {
			...
			public <T> void forEachEvent(final DebugSubscription<T> subscription, final EventVisitor<T> visitor) {
	...
	public <T> void updateChunk(final long gameTime, final ChunkPos chunkPos, final Update<T> update) {
		this.updateMap(gameTime, chunkPos, update, chunks());
	}

	public <T> void updateBlock(final long gameTime, final BlockPos blockPos, final Update<T> update) {
		this.updateMap(gameTime, blockPos, update, blocks());
	}

	public <T> void updateEntity(final long gameTime, final Entity entity, final Update<T> update) {
		this.updateMap(gameTime, entity.getUUID(), update, entities());
	}

	public <T> void pushEvent(final long gameTime, final Event<T> event) {
	...
	private <K, V> void updateMap(final long gameTime, final K key, final Update<V> update, final ClientDebugSubscriber.ValueMapType<K, V> type) {
	...
	private static class ValueMap<K, V> {
		...
		public void apply(final long gameTime, final K key, final Update<V> update) {
