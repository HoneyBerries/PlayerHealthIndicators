...
public class ChatLog {
			...
			.comapFlatMap(
				loggedChatEvents -> {
					...
					return parsedSize > capacity
						? DataResult.error(() -> "Expected: a buffer of size less than or equal to " + capacity + " but: " + parsedSize + " is greater than " + capacity)
	...
	private ChatLog(final int capacity, final List<LoggedChatEvent> buffer) {
		this.buffer = (LoggedChatEvent[])buffer.toArray(LoggedChatEvent[]::new);
	...
	@Nullable
	public LoggedChatEvent lookup(final int id) {
