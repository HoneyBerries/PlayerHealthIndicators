...
public class EnderDragonModel extends EntityModel<EnderDragonRenderState> {
	...
	public void setupAnim(final EnderDragonRenderState state) {
		...
		float rotScale = 1.5F;
		Sample start = state.getHistoricalPos(6);
		...
		for (int i = 0; i < 5; i++) {
			...
			Sample point = state.getHistoricalPos(5 - i);
		...
		this.head.z = zz;
		this.head.x = xx;
		Sample current = state.getHistoricalPos(0);
		...
		for (int i = 0; i < 12; i++) {
			Sample point = state.getHistoricalPos(12 + i);
