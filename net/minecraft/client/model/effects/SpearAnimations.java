...
public class SpearAnimations {
	...
	record UseParams(
		...
		public static SpearAnimations.UseParams fromKineticWeapon(final KineticWeapon kineticWeapon, final float time) {
			...
			int finishSwayingTick = (Integer)kineticWeapon.dismountConditions().map(Condition::maxDurationTicks).orElse(0) + finishRaisingTick;
			int startSwayingTick = finishSwayingTick - 20;
			int finishLoweringTick = (Integer)kineticWeapon.knockbackConditions().map(Condition::maxDurationTicks).orElse(0) + finishRaisingTick;
			int startLoweringTick = finishLoweringTick - 40;
			int finishRaisingBackTick = (Integer)kineticWeapon.damageConditions().map(Condition::maxDurationTicks).orElse(0) + finishRaisingTick;
