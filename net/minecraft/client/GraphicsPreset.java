...
public enum GraphicsPreset implements StringRepresentable {
	...
	public void apply(final Minecraft minecraft) {
		...
		switch (this) {
			...
			case FABULOUS: {
				...
				this.set(screen, minecraft.options.cutoutLeaves(), true);
				this.set(screen, minecraft.options.improvedTransparency(), Util.getPlatform() != OS.OSX);
				this.set(screen, minecraft.options.weatherRadius(), 10);
	...
	<T> void set(@Nullable final OptionsSubScreen screen, final OptionInstance<T> option, final T value) {
		if (option.get() != value) {
			option.set(value);
			if (screen != null) {
