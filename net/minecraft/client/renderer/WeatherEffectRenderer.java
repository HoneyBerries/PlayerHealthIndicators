...
public class WeatherEffectRenderer {
	...
	public void tickRainParticles(final ClientLevel level, final Camera camera, final int ticks, final ParticleStatus particleStatus, final int weatherRadius) {
		...
		if (!(rainLevel <= 0.0F)) {
			...
			for (int ii = 0; ii < rainParticles; ii++) {
				...
				BlockPos heightmapPosition = level.getHeightmapPos(Types.MOTION_BLOCKING, cameraPosition.offset(x, 0, z));
				if (heightmapPosition.getY() > level.getMinY()
					&& heightmapPosition.getY() <= cameraPosition.getY() + 10
					&& heightmapPosition.getY() >= cameraPosition.getY() - 10
					&& this.getPrecipitationAt(level, heightmapPosition) == Precipitation.RAIN) {
			...
			if (rainParticlePosition != null && random.nextInt(3) < this.rainSoundTime++) {
				...
				if (rainParticlePosition.getY() > cameraPosition.getY() + 1
					&& level.getHeightmapPos(Types.MOTION_BLOCKING, cameraPosition).getY() > Mth.floor(cameraPosition.getY())) {
	...
	private Precipitation getPrecipitationAt(final Level level, final BlockPos pos) {
