...
public class BlockModelLighter {
	...
	public BlockModelLighter() {
		this.cache = (BlockModelLighter.Cache)CACHE.get();
	}
	...
	public void prepareQuadFlat(
		final BlockAndTintGetter level, final BlockState state, final BlockPos pos, final int lightCoords, final BakedQuad quad, final QuadInstance outputInstance
	) {
		if (lightCoords == -1) {
	...
	public static void enableCaching() {
		((BlockModelLighter.Cache)CACHE.get()).enable();
	}

	public static void clearCache() {
		((BlockModelLighter.Cache)CACHE.get()).disable();
	}
	...
	private static enum AdjacencyInfo {
		...
		private static final BlockModelLighter.AdjacencyInfo[] BY_FACING = (BlockModelLighter.AdjacencyInfo[])Util.make(
			new BlockModelLighter.AdjacencyInfo[6], map -> {
	...
	private static enum AmbientVertexRemap {
		...
		private static final BlockModelLighter.AmbientVertexRemap[] BY_FACING = (BlockModelLighter.AmbientVertexRemap[])Util.make(
			new BlockModelLighter.AmbientVertexRemap[6], map -> {
				map[Direction.DOWN.get3DDataValue()] = DOWN;
				map[Direction.UP.get3DDataValue()] = UP;
				map[Direction.NORTH.get3DDataValue()] = NORTH;
				map[Direction.SOUTH.get3DDataValue()] = SOUTH;
