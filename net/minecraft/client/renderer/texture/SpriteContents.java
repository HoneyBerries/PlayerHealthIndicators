...
public class SpriteContents implements AutoCloseable, Stitcher.Entry {
	...
	public SpriteContents(
		...
		final List<WithValue<?>> additionalMetadata,
		...
		this.animatedTexture = (SpriteContents.AnimatedTexture)animationInfo.map(
	...
	private SpriteContents.AnimatedTexture createAnimatedTexture(
		...
		if (metadata.frames().isEmpty()) {
			...
			List<AnimationFrame> metadataFrames = (List<AnimationFrame>)metadata.frames().get();
			...
			for (Iterator<SpriteContents.FrameInfo> iterator = frames.iterator(); iterator.hasNext(); index++) {
				SpriteContents.FrameInfo frame = (SpriteContents.FrameInfo)iterator.next();
	...
	public <T> Optional<T> getAdditionalMetadata(final MetadataSectionType<T> type) {
		for (WithValue<?> metadata : this.additionalMetadata) {
	...
	public class AnimationState implements AutoCloseable {
		...
		public void tick() {
			this.subFrame++;
			this.isDirty = false;
			SpriteContents.FrameInfo currentFrame = (SpriteContents.FrameInfo)this.animationInfo.frames.get(this.frame);
			if (this.subFrame >= currentFrame.time) {
				int oldFrame = currentFrame.index;
				this.frame = (this.frame + 1) % this.animationInfo.frames.size();
				this.subFrame = 0;
				int newFrame = ((SpriteContents.FrameInfo)this.animationInfo.frames.get(this.frame)).index;
		...
		public void drawToAtlas(final RenderPass renderPass, final GpuBufferSlice ubo) {
			...
			int oldFrame = ((SpriteContents.FrameInfo)frames.get(this.frame)).index;
			float frameProgress = (float)this.subFrame / ((SpriteContents.FrameInfo)this.animationInfo.frames.get(this.frame)).time;
			int frameProgressAsInt = (int)(frameProgress * 1000.0F);
			if (this.animationInfo.interpolateFrames) {
				int newFrame = ((SpriteContents.FrameInfo)frames.get((this.frame + 1) % frames.size())).index;
