...
public class QuadParticleRenderState implements ParticleGroupRenderState, SubmitNodeCollector.ParticleGroupRenderer {
	...
	public void add(
		...
		((QuadParticleRenderState.Storage)this.particles.computeIfAbsent(layer, ignored -> new QuadParticleRenderState.Storage()))
	...
	@Nullable
	@Override
	public QuadParticleRenderState.PreparedBuffers prepare(final ParticleFeatureRenderer.ParticleBufferCache cachedBuffer, final boolean translucent) {
		if (this.isEmpty()) {
			return null;
		} else {
				...
				for (Entry<SingleQuadParticle.Layer, QuadParticleRenderState.Storage> entry : this.particles.entrySet()) {
					if (((SingleQuadParticle.Layer)entry.getKey()).translucent() == translucent) {
						...
						if (((QuadParticleRenderState.Storage)entry.getValue()).count() > 0) {
							preparedLayers.put(
								(SingleQuadParticle.Layer)entry.getKey(),
								new QuadParticleRenderState.PreparedLayer(offset, ((QuadParticleRenderState.Storage)entry.getValue()).count() * 6)
							);
						}

						offset += ((QuadParticleRenderState.Storage)entry.getValue()).count() * 4;
					}
				}
