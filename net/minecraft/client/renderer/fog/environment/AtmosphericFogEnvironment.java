...
public class AtmosphericFogEnvironment extends FogEnvironment {
	...
	public int getBaseColor(final ClientLevel level, final Camera camera, final int renderDistance, final float partialTicks) {
		...
		if (renderDistance >= 4) {
			float sunAngle = (Float)camera.attributeProbe().getValue(EnvironmentAttributes.SUN_ANGLE, partialTicks) * (float) (Math.PI / 180.0);
		...
		float skyFogEnd = Math.min((Float)camera.attributeProbe().getValue(EnvironmentAttributes.SKY_FOG_END_DISTANCE, partialTicks) / 16.0F, renderDistance);
	...
	public void setupFog(final FogData fog, final Camera camera, final ClientLevel level, final float renderDistance, final DeltaTracker deltaTracker) {
		...
		fog.cloudEnd = Math.min(
			Minecraft.getInstance().options.cloudRange().get() * 16, (Float)camera.attributeProbe().getValue(EnvironmentAttributes.CLOUD_FOG_END_DISTANCE, partialTicks)
		);
