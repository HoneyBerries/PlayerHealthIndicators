...
public class GameEventListenerRenderer implements DebugRenderer.SimpleDebugRenderer {
	...
	public void emitGizmos(
		...
		this.forEachListener(debugValues, (origin, radius) -> {
			Gizmos.billboardText("Listener Origin", origin.add(0.0, 1.8, 0.0), Style.whiteAndCentered().withScale(0.4F));
			Gizmos.billboardText(BlockPos.containing(origin).toString(), origin.add(0.0, 1.5, 0.0), Style.forColorAndCentered(-6959665).withScale(0.4F));
		});
		debugValues.forEachEvent(DebugSubscriptions.GAME_EVENTS, (event, remainingTicks, totalLifetime) -> {
			...
			Gizmos.billboardText(event.event().getRegisteredName(), origin.add(0.0, 0.85, 0.0), Style.forColorAndCentered(-7564911).withScale(0.12F));
		});
	}
