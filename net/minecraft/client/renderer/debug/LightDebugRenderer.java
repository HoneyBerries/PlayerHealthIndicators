		...
		for (BlockPos blockPos : BlockPos.betweenClosed(playerPos.offset(-10, -10, -10), playerPos.offset(10, 10, 10))) {
			...
			if (set.add(sectionNode)) {
				Gizmos.billboardText(
					...
					Style.forColorAndCentered(-65536).withScale(4.8F)
				);
			}

			if (skyBrightness != 15 && this.showSkyLight) {
				int color = ARGB.srgbLerp(skyBrightness / 15.0F, -16776961, -16711681);
				Gizmos.billboardText(String.valueOf(skyBrightness), Vec3.atLowerCornerWithOffset(blockPos, 0.5, 0.25, 0.5), Style.forColorAndCentered(color));
			}
			...
			if (this.showBlockLight) {
				...
				if (blockBrightness != 0) {
					...
					Gizmos.billboardText(String.valueOf(level.getBrightness(LightLayer.BLOCK, blockPos)), Vec3.atCenterOf(blockPos), Style.forColorAndCentered(color));
				}
			}
		}
	}
