...
public class OctreeDebugRenderer implements DebugRenderer.SimpleDebugRenderer {
	...
	private void renderNode(final Octree.Node node, final int depth, final boolean fullyVisible, final MutableInt count, final boolean isClose) {
		...
		if (size == 1L) {
			...
			Gizmos.billboardText(String.valueOf(count.intValue()), aabb.getCenter(), Style.forColorAndCentered(color).withScale(4.8F));
		}
		...
		Gizmos.cuboid(
			...
			GizmoStyle.stroke(
				ARGB.colorFromFloat(fullyVisible ? 0.4F : 1.0F, getColorComponent(colorNum, 0.3F), getColorComponent(colorNum, 0.8F), getColorComponent(colorNum, 0.5F))
			)
		);
	}
