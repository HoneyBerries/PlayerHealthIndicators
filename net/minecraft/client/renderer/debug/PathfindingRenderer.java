	...
	public static void renderPath(
		...
		if (renderGroundLabels) {
			for (int ix = 0; ix < path.getNodeCount(); ix++) {
				...
				if (distanceToCamera(n.asBlockPos(), camX, camY, camZ) <= 80.0F) {
					Gizmos.billboardText(String.valueOf(n.type), new Vec3(n.x + 0.5, n.y + 0.75, n.z + 0.5), Style.whiteAndCentered().withScale(0.32F)).setAlwaysOnTop();
					Gizmos.billboardText(
							String.format(Locale.ROOT, "%.2f", n.costMalus), new Vec3(n.x + 0.5, n.y + 0.25, n.z + 0.5), Style.whiteAndCentered().withScale(0.32F)
						)
						.setAlwaysOnTop();
				}
			}
