...
public class EnderDragonRenderer extends EntityRenderer<EnderDragon, EnderDragonRenderState> {
		...
		submitNodeCollector.submitCustomGeometry(
			...
			(pose, buffer) -> {
				...
				for (int i = 0; i < rayCount; i++) {
					rayRotation.rotationXYZ(random.nextFloat() * (float) (Math.PI * 2), random.nextFloat() * (float) (Math.PI * 2), random.nextFloat() * (float) (Math.PI * 2))
						.rotateXYZ(
	...
	public void extractRenderState(final EnderDragon entity, final EnderDragonRenderState state, final float partialTicks) {
		...
		BlockPos egg = entity.level().getHeightmapPos(Types.MOTION_BLOCKING_NO_LEAVES, EndPodiumFeature.getLocation(entity.getFightOrigin()));
