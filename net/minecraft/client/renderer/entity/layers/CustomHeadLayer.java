...
public class CustomHeadLayer<S extends LivingEntityRenderState, M extends EntityModel<S> & HeadedModel> extends RenderLayer<S, M> {
	...
	public void submit(
		...
		if (!state.headItem.isEmpty() || state.wornHeadType != null) {
			...
			if (state.wornHeadType != null) {
				...
				SkullBlockRenderer.submitSkull(state.wornHeadAnimationPos, poseStack, submitNodeCollector, lightCoords, skullModel, renderType, state.outlineColor, null);
			} else {
	...
	private RenderType resolveSkullRenderType(final LivingEntityRenderState state, final Type type) {
