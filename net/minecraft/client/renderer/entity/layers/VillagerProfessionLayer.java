...
public class VillagerProfessionLayer<S extends LivingEntityRenderState & VillagerDataHolderRenderState, M extends EntityModel<S> & VillagerLikeModel<S>>
	...
	private static final Int2ObjectMap<Identifier> LEVEL_LOCATIONS = (Int2ObjectMap<Identifier>)Util.make(new Int2ObjectOpenHashMap(), map -> {
	...
	public <K> VillagerMetadataSection.Hat getHatData(
		...
		ResourceKey<K> key = (ResourceKey<K>)holder.unwrapKey().orElse(null);
		return key == null
			? VillagerMetadataSection.Hat.NONE
			: cache.computeIfAbsent(
				key, k -> (VillagerMetadataSection.Hat)this.resourceManager.getResource(this.getIdentifier(name, key.identifier())).flatMap(resource -> {
					try {
