...
public class LlamaDecorLayer extends RenderLayer<LlamaRenderState, LlamaModel> {
	...
	public void submit(
		...
		if (equippable != null && equippable.assetId().isPresent() && !state.isBaby) {
			this.renderEquipment(poseStack, submitNodeCollector, state, itemStack, (ResourceKey<EquipmentAsset>)equippable.assetId().get(), lightCoords);
		} else if (state.isTraderLlama) {
			this.renderEquipment(
				poseStack, submitNodeCollector, state, ItemStack.EMPTY, state.isBaby ? EquipmentAssets.TRADER_LLAMA_BABY : EquipmentAssets.TRADER_LLAMA, lightCoords
			);
		}
	}
	...
	private void renderEquipment(
		...
		this.equipmentRenderer
			.renderLayers(
				EquipmentClientInfo.LayerType.LLAMA_BODY, equipmentAssetId, model, state, itemStack, poseStack, submitNodeCollector, lightCoords, state.outlineColor
			);
	}
}
