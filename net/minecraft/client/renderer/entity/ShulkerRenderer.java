...
public class ShulkerRenderer extends MobRenderer<Shulker, ShulkerRenderState, ShulkerModel> {
	...
	private static final Identifier[] TEXTURE_LOCATION = (Identifier[])Sheets.SHULKER_TEXTURE_LOCATION
		.stream()
		...
		if (super.shouldRender(entity, culler, camX, camY, camZ)) {
			...
			if (startPos == null) {
				...
				return culler.isVisible(
					new AABB(startPos.x, startPos.y + halfHeight, startPos.z, targetPos.x, targetPos.y + halfHeight, targetPos.z).inflate(halfWidth, halfHeight, halfWidth)
				);
			}
		}
	}
