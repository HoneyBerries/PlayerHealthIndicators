...
public abstract class DisplayRenderer<T extends Display, S, ST extends DisplayEntityRenderState> extends EntityRenderer<T, ST> {
	...
	private Quaternionf calculateOrientation(final RenderState renderState, final ST state, final Quaternionf output) {
	...
	public static class BlockDisplayRenderer extends DisplayRenderer<BlockDisplay, BlockRenderState, BlockDisplayEntityRenderState> {
		...
		public void extractRenderState(final BlockDisplay entity, final BlockDisplayEntityRenderState state, final float partialTicks) {
			super.extractRenderState((T)entity, state, partialTicks);
			BlockRenderState blockRenderState = entity.blockRenderState();
	...
	public static class ItemDisplayRenderer extends DisplayRenderer<ItemDisplay, ItemRenderState, ItemDisplayEntityRenderState> {
		...
		public void extractRenderState(final ItemDisplay entity, final ItemDisplayEntityRenderState state, final float partialTicks) {
			super.extractRenderState((T)entity, state, partialTicks);
			ItemRenderState itemRenderState = entity.itemRenderState();
	...
	public static class TextDisplayRenderer extends DisplayRenderer<TextDisplay, TextRenderState, TextDisplayEntityRenderState> {
		...
		public void extractRenderState(final TextDisplay entity, final TextDisplayEntityRenderState state, final float partialTicks) {
			super.extractRenderState((T)entity, state, partialTicks);
			state.textRenderState = entity.textRenderState();
			state.cachedInfo = entity.cacheDisplay(this::splitLines);
		}

		private CachedInfo splitLines(final Component input, final int width) {
			...
			for (FormattedCharSequence line : lines) {
				...
				result.add(new CachedLine(line, lineWidth));
			}

			return new CachedInfo(result, maxLineWidth);
		}
		...
		public void submitInner(
			...
			CachedInfo cachedInfo = state.cachedInfo;
			int lineSpacing = 1;
			int lineHeight = 9 + 1;
			...
			for (CachedLine line : cachedInfo.lines()) {
