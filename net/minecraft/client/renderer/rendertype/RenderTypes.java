...
public class RenderTypes {
	...
	private static final Function<Identifier, RenderType> CRUMBLING = Util.memoize(
		texture -> RenderType.create("crumbling", RenderSetup.builder(RenderPipelines.CRUMBLING).withTexture("Sampler0", texture).sortOnUpload().createRenderSetup())
	);
	...
	private static final Function<Identifier, RenderType> TEXT_INTENSITY = Util.memoize(
		texture -> RenderType.create(
			"text_intensity", RenderSetup.builder(RenderPipelines.TEXT_INTENSITY).withTexture("Sampler0", texture).useLightmap().bufferSize(786432).createRenderSetup()
		)
	);
