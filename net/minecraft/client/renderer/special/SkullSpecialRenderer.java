...
public class SkullSpecialRenderer implements NoDataSpecialModelRenderer {
	...
	public record Unbaked(Type kind, Optional<Identifier> textureOverride, float animation) implements NoDataSpecialModelRenderer.Unbaked {
		...
		public Unbaked(final Type kind) {
		...
		public SkullSpecialRenderer bake(final SpecialModelRenderer.BakingContext context) {
			...
			Identifier textureOverride = (Identifier)this.textureOverride.map(t -> t.withPath(p -> "textures/entity/" + p + ".png")).orElse(null);
			if (model == null) {
				return null;
			} else {
