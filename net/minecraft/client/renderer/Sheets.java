...
public class Sheets {
	...
	public static final Map<ResourceKey<DecoratedPotPattern>, SpriteId> DECORATED_POT_SPRITES = (Map<ResourceKey<DecoratedPotPattern>, SpriteId>)BuiltInRegistries.DECORATED_POT_PATTERN
		.listElements()
		.collect(Collectors.toMap(Reference::key, holder -> DECORATED_POT_MAPPER.apply(((DecoratedPotPattern)holder.value()).assetId())));
	...
	public static SpriteId getBannerSprite(final Holder<BannerPattern> pattern) {
		return (SpriteId)BANNER_SPRITES.computeIfAbsent(((BannerPattern)pattern.value()).assetId(), BANNER_MAPPER::apply);
	}

	public static SpriteId getShieldSprite(final Holder<BannerPattern> pattern) {
		return (SpriteId)SHIELD_SPRITES.computeIfAbsent(((BannerPattern)pattern.value()).assetId(), SHIELD_MAPPER::apply);
	}
