...
public class SoundManager extends SimplePreparableReloadListener<SoundManager.Preparations> {
	...
	protected SoundManager.Preparations prepare(final ResourceManager manager, final ProfilerFiller profiler) {
		...
		Zone ignored = profiler.zone("list");
		...
		for (String namespace : manager.getNamespaces()) {
			try {
				Zone ignoredx = profiler.zone(namespace);
	...
	@Nullable
	public WeighedSoundEvents getSoundEvent(final Identifier location) {
					...
					case SOUND_EVENT:
						weighted = new Weighted<Sound>() {
							...
							public int getWeight() {
								WeighedSoundEvents registrationx = (WeighedSoundEvents)Preparations.this.registry.get(soundLocation);
							...
							public Sound getSound(final RandomSource random) {
								...
								if (registrationx == null) {
									...
									return new Sound(
										...
										new MultipliedFloats(new SampledFloat[]{wrappedSound.getVolume(), sound.getVolume()}),
										new MultipliedFloats(new SampledFloat[]{wrappedSound.getPitch(), sound.getPitch()}),
										sound.getWeight(),
										Sound.Type.FILE,
